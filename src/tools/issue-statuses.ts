...
import { defineTool, IssueStatusListSchema, IssueStatusQuerySchema } from '../schemas/index.js';
...
export const listIssueStatusesTool = defineTool({
  ...
  handler: async ({ teamId }) => {
    ...
    try {
      ...
      if (!team) {
        throw new McpError(ErrorCode.MethodNotFound, `Team with ID ${teamId} not found`);
      }
...
export const getIssueStatusTool = defineTool({
  ...
  handler: async ({ query, teamId }) => {
    ...
    try {
      ...
      throw new McpError(ErrorCode.MethodNotFound, `Issue status with ID or name "${query}" not found in team ${teamId}`);
    } catch (error: unknown) {
      const err = error as { message?: string };
      throw new McpError(
        ErrorCode.InternalError,
