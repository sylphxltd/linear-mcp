...
async function getAvailableProjectMilestonesJson(
  ...
  try {
    ...
    if (projectId && projectId !== 'any' && projectId.trim() !== '') {
      ...
      const projectList = allProjects.nodes.map((p) => ({
        id: p.id,
        name: p.name,
      }));
    ...
    const milestoneList = milestones.nodes.map((milestone) => ({
      id: milestone.id,
      name: milestone.name,
    }));
      ...
      if (stateId && teamId) {
        try {
          ...
          if (!team) {
            ...
            throw new McpError(
              ErrorCode.InvalidParams,
              `Cannot validate stateId: Team '${teamId}' not found. Valid teams are: ${availableTeamsJson}`,
            );
          }
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `Cannot validate stateId: 'teamId' is required when 'stateId' is provided. Valid teams are: ${availableTeamsJson}`,
        );
      }
        ...
        issuesConnection.nodes.map(async (issueNode: Issue) => {
          ...
          return {
            ...
            projectMilestone: projectMilestoneEntity
              ? {
                  id: projectMilestoneEntity.id,
                  name: projectMilestoneEntity.name,
                }
              : null,
      ...
      if (!issue) {
        ...
        try {
          ...
          if (recentIssues.nodes.length > 0) {
            const issueList = recentIssues.nodes.map((iss) => ({
              id: iss.id,
              title: iss.title,
    ...
    try {
      ...
      if (stateId && teamId) {
        try {
          const team = await linearClient.team(teamId);
          if (!team) {
            throw new McpError(ErrorCode.InternalError, 'Team validation failed unexpectedly.');
          }
          const states = await team.states({ filter: { id: { eq: stateId } } });
          if (!states.nodes || states.nodes.length === 0) {
            const availableStatesJson = await getAvailableStatesJson(linearClient, teamId);
      ...
      if (labelIds && labelIds.length > 0 && teamId) {
        try {
          ...
          if (!team) {
            throw new McpError(
              ErrorCode.InternalError,
              'Team validation failed unexpectedly for labels.',
            );
          }
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          'teamId is required when labelIds are provided.',
        );
      }
      ...
      if (projectMilestoneId) {
        try {
          const projectMilestone = await linearClient.projectMilestone(projectMilestoneId);
          if (!projectMilestone) {
            const availableMilestonesJson = await getAvailableProjectMilestonesJson(
              linearClient,
              projectId,
            );
            throw new McpError(
          ...
          if (projectId && projectMilestone.projectId !== projectId) {
            ...
            const availableMilestonesForGivenProjectJson = await getAvailableProjectMilestonesJson(
              linearClient,
              projectId,
            );
            throw new McpError(
    ...
    try {
      ...
      if (stateId) {
        if (!currentTeamId) {
          throw new McpError(
            ErrorCode.InternalError,
            `Issue '${id}' does not have a team associated, cannot validate stateId. Please ensure the issue is associated with a team or provide a teamId if changing teams.`,
          );
        }
        try {
          ...
          if (!team) {
            throw new McpError(
              ErrorCode.InternalError,
              `Team with ID '${currentTeamId}' for issue '${id}' not found unexpectedly.`,
            );
          }
          const states = await team.states({ filter: { id: { eq: stateId } } });
          if (!states.nodes || states.nodes.length === 0) {
      ...
      if (labelIds && labelIds.length > 0) {
        if (!currentTeamId) {
          throw new McpError(
            ErrorCode.InternalError,
            `Issue '${id}' does not have a team associated, cannot validate labelIds. Please ensure the issue is associated with a team.`,
          );
        }
        try {
          const team = await linearClient.team(currentTeamId);
        ...
        if (projectMilestoneId) {
          try {
            ...
            if (issueCurrentProjectId && projectMilestone.projectId !== issueCurrentProjectId) {
              ...
              const availableMilestonesForIssueProjectJson =
                await getAvailableProjectMilestonesJson(linearClient, issueCurrentProjectId);
              throw new McpError(
                ErrorCode.InvalidParams,
...
export const listCommentsTool = defineTool({
  ...
  inputSchema: {
    issueId: z.string().describe('The ID of the issue to fetch comments for'),
  },
  handler: async ({ issueId }) => {
    try {
      ...
      if (!issue) {
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `Issue with ID '${issueId}' not found when trying to list comments.${recentIssuesMessage}`,
        );
      }
  ...
  handler: async ({ issueId, body }) => {
    try {
      ...
      if (!issue) {
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `Issue with ID '${issueId}' not found when trying to create comment.${recentIssuesMessage}`,
        );
      }
  ...
  handler: async ({ id }) => {
    try {
      ...
      if (!issue) {
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `Issue with ID '${id}' not found when trying to get branch name.${recentIssuesMessage}`,
        );
      }
