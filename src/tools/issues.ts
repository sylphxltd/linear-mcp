...
import type { LinearClient, Issue, WorkflowState, User, Team, Project, Comment as LinearComment, IssuePayload, CommentPayload } from '@linear/sdk';
import {
  defineTool,
  IdSchema,
  IssueFilterSchema,
...
export const listIssuesTool = defineTool({
  ...
  handler: async (args) => {
    try {
      const { query, teamId, stateId, assigneeId, includeArchived = true, limit = 50 } = args;
      const filters: IssueFilters = {};
      if (query) filters.filter = { or: [{ title: { containsIgnoreCase: query } }, { description: { containsIgnoreCase: query } }] };
      if (teamId) filters.teamId = teamId;
      if (stateId) filters.stateId = stateId;
      if (assigneeId) filters.assigneeId = assigneeId;
      const linearClient = getLinearClient();
      const issuesConnection = await linearClient.issues(filters as Parameters<typeof linearClient.issues>[0]);
      const issues = await Promise.all(issuesConnection.nodes.map(async (issueNode: Issue) => {
...
export const getIssueTool = defineTool({
  ...
  handler: async (args) => {
    try {
      ...
      if (!issue) {
        throw new McpError(ErrorCode.MethodNotFound, `Issue with ID ${id} not found`);
      }
...
export const createIssueTool = defineTool({
  ...
  handler: async (args) => {
    try {
      ...
      const issueCreateInput: Parameters<typeof LinearClient.prototype.createIssue>[0] = {
        title,
        description,
        teamId,
      };
...
export const updateIssueTool = defineTool({
  ...
  handler: async (args) => {
    try {
      ...
      throw new McpError(ErrorCode.InternalError, 'Failed to update issue: No issue returned');
    } catch (error: unknown) {
      const err = error as { message?: string };
      throw new McpError(
        ErrorCode.InternalError,
...
export const listCommentsTool = defineTool({
  ...
  handler: async ({ issueId }) => {
    ...
    try {
      ...
      if (!issue) {
        throw new McpError(ErrorCode.MethodNotFound, `Issue with ID ${issueId} not found`);
      }
...
export const createCommentTool = defineTool({
  ...
  handler: async ({ issueId, body }) => {
    ...
    try {
      ...
      throw new McpError(ErrorCode.InternalError, 'Failed to create comment: No comment returned');
    } catch (error: unknown) {
      const err = error as { message?: string };
      throw new McpError(
        ErrorCode.InternalError,
